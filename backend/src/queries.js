...
export const q_getHomePageFreelancers = "select concat(fname,' ',lname) as 'title' , concat(city,', ',country) as 'subtitle'  from Freelancer"
export const q_getHomePageJobs = "select j.job_id, c.name as 'subtitle', j.name as 'title' from Jobs j join Company c on c.company_id = j.company_id join Job_Status js on js.id = j.status_id where js.id=1"
